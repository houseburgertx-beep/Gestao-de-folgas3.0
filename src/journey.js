...
function managerCommand() {
 ...
 const priority=[
  {count:requests,label:requests===1?'solicitação aguardando decisão':'solicitações aguardando decisão',detail:[data.pendingCounts?.timeOff&&`${data.pendingCounts.timeOff} folgas`,data.pendingCounts?.clock&&`${data.pendingCounts.clock} pontos`,data.pendingCounts?.swaps&&`${data.pendingCounts.swaps} trocas`].filter(Boolean).join(' · '),view:'pending-center',action:'Revisar'},
  {count:incomplete,label:incomplete===1?'saída esquecida':'saídas esquecidas',detail:'Jornadas anteriores sem fechamento',view:'pending-center',action:'Corrigir ponto'},
  {count:late,label:late===1?'entrada atrasada':'entradas atrasadas',detail:latePeople.slice(0,2).map(p=>`${p.Nome} · ${delay(p)} min após`).join(' / '),view:'live-team',action:'Ver equipe'},
  {count:missing,label:missing===1?'pessoa ainda sem entrada':'pessoas ainda sem entrada',detail:missingPeople.slice(0,3).map(p=>p.Nome).join(', '),view:'live-team',action:'Conferir'},
 ].filter(item=>item.count>0);
 const currentDate=new Date().toLocaleDateString('pt-BR',{timeZone:'America/Bahia',weekday:'long',day:'2-digit',month:'long'});
 return `<section class="j-command"><div class="j-command-head"><div><span class="eyebrow">OPERAÇÃO DE HOJE</span><h2>O que precisa da sua decisão</h2><p>${esc(currentDate)}</p></div>${unitSelect()}</div><div class="j-priority-list">${priority.map(item=>`<button data-view-target="${item.view}"><strong>${item.count}</strong><span>${esc(item.label)}</span>${item.detail?`<small>${esc(item.detail)}</small>`:''}<b>${item.action} →</b></button>`).join('') || '<div class="j-all-clear"><span>✓</span><div><strong>Nenhuma pendência crítica agora</strong><small>A equipe está dentro do previsto para esta seleção.</small></div></div>'}</div><div class="j-command-actions"><button class="btn btn-secondary" data-view-target="calendar">Abrir calendário</button><button class="btn btn-primary" data-view-target="scheduler">Planejar próximas folgas</button></div></section>`;
}
function operation() {
