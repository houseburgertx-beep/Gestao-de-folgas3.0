...
function operation() {
 ...
 const alerts=r.filter(p=>p.alertMessage || delay(p)>0).sort((a,b)=>(b.alertLevel==='danger')-(a.alertLevel==='danger'));
 return `<div class="j-stats">${counts}</div><article class="j-card j-team-chart"><div class="j-card-top"><h3>Equipe agora</h3><span class="j-live"><i></i> AO VIVO</span></div><div class="j-donut-wrap"><div class="j-donut" role="img" aria-label="${pct}% da equipe presente" style="background:conic-gradient(${r.length?gradient:'#eceaf2 0% 100%'})"><div><strong>${data.presence?pct+'%':'—'}</strong><small>Presentes</small></div></div><div class="j-legend">${Object.entries(names).map(([k,n])=>`<span><i style="background:${colors[k]}"></i>${n}<b>${r.filter(p=>p.status===k).length}</b></span>`).join('')}</div></div><small>Presentes = trabalhando + intervalo. Sem entrada não confirma falta.</small></article><article class="j-card j-people"><div class="j-card-top"><div><h3>Agora na empresa</h3><small>${present} de ${r.length} colaboradores presentes</small></div><div class="j-avatar-stack">${r.filter(p=>['trabalhando','intervalo'].includes(p.status)).slice(0,4).map(avatar).join('')}</div></div>${r.filter(p=>['trabalhando','intervalo'].includes(p.status)).slice(0,3).map(person).join('') || '<p class="j-empty">Nenhum colaborador presente nesta seleção.</p>'}<button class="btn btn-ghost" data-view-target="live-team">Ver toda a equipe →</button></article><article class="j-card"><div class="j-card-top"><h3>Atividade ao vivo</h3><span class="j-live"><i></i></span></div><p class="j-caption">Última marcação de cada pessoa</p><div class="j-activity">${recent.map(p=>`<div><time>${esc(p.lastPunchTime)}</time><i></i><span><strong>${esc(p.Nome)}</strong><small>${esc({ENTRADA:'registrou entrada',SAIDA_INTERVALO:'iniciou intervalo',RETORNO_INTERVALO:'voltou do intervalo',SAIDA_FINAL:'encerrou a jornada'}[p.lastPunchType] || p.statusLabel)}</small></span></div>`).join('') || '<p class="j-empty">Nenhuma marcação disponível.</p>'}</div></article><article class="j-card j-attention"><h3>Precisa da sua atenção</h3>${alerts.slice(0,3).map(p=>`<p><strong>${esc(p.Nome)}</strong><br>${esc(p.alertMessage || `${delay(p)} min após o horário previsto`)}</p>`).join('') || '<p>Nenhum alerta de jornada nesta seleção.</p>'}<button class="btn btn-secondary" data-view-target="pending-center">Revisar solicitações →</button></article>`;
}
function personal() {
...
document.addEventListener('click',e=>{const f=e.target.closest('[data-j-filter]'),m=e.target.closest('[data-j-mode]');if(f){teamFilter=f.dataset.jFilter;renderTeam();}if(m){mode=m.dataset.jMode;renderSchedule();}});
